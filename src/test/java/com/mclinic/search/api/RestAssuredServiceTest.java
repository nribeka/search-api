...
public class RestAssuredServiceTest {
    ...
    public void loadObjects_shouldLoadObjectFromFilesystemBasedOnTheResourceDescription() throws Exception {
        ...
        Patient patient = service.getObject("familyName: " + StringUtil.quote(patientFamilyName), Patient.class);
        Assert.assertNotNull(patient);
        Assert.assertEquals(patientGivenName, patient.getGivenName());
        Assert.assertEquals(patientFamilyName, patient.getFamilyName());
    }
    ...
    public void getObject_shouldThrowIOExceptionIfTheKeyAndClassUnableToReturnUniqueObject() throws Exception {
        Patient patient = service.getObject("givenName: T*", Patient.class);
        Assert.assertNull(patient);
    }
    ...
    public void getObject_shouldThrowIOExceptionIfTheKeyAndResourceUnableToReturnUniqueObject() throws Exception {
        Resource resource = context.getResource(PATIENT_RESOURCE);
        Patient patient = (Patient) service.getObject("givenName: T*", resource);
        Assert.assertNull(patient);
    }
...
//        String baseUri = StringUtil.escapeUri("http://149.166.10.181:8081/openmrs-standalone/ws/rest/v1/patient/");
